...
const (
	...
	OwnedByLabel   = "v1.custom-resources-selfadapter.net/owned-by"
)

type K8sReconciler interface {
	Reconcile(
		reqLogger logr.Logger,
		instance *customresourcesselfdapternetv1.CustomResourcesSelfadapter,
		obj metav1.Object,
		shouldProvision bool,
		updateable bool,
		kind string,
	) (reconcile.Result, error)
	PodCleanup(reqLogger logr.Logger, instance *customresourcesselfdapternetv1.CustomResourcesSelfadapter) error
}
...
// CustomResourcesSelfadapterReconciler reconciles a CustomResourcesSelfadapter object
type CustomResourcesSelfadapterReconciler struct {
	client.Client
	Log logr.Logger
...
// +kubebuilder:rbac:groups=customresourcesselfdapter.net.custom-resources-selfadapter.net,resources=customresourcesselfadapters,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=customresourcesselfdapter.net.custom-resources-selfadapter.net,resources=customresourcesselfadapters/status,verbs=get;update;patch
// +kubebuilder:rbac:groups=customresourcesselfdapter.net.custom-resources-selfadapter.net,resources=customresourcesselfadapters/finalizers,verbs=update

// Reconcile is part of the main kubernetes reconciliation loop which aims to
// move the current state of the cluster closer to the desired state.
// TODO(user): Modify the Reconcile function to compare the state specified by
// the CustomResourcesSelfadapter object against the actual cluster state, and then
// perform operations to make the cluster state reflect the state specified by
// the user.
//
// For more details, check Reconcile and its Result here:
// - https://pkg.go.dev/sigs.k8s.io/controller-runtime@v0.21.0/pkg/reconcile
func (r *CustomResourcesSelfadapterReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
...
instance := &customresourcesselfdapternetv1.CustomResourcesSelfadapter{}
	...
	for _, container := range podSpec.Containers {
		...
		envVars = append(envVars, crsEnvVars(instance, string(scaleTargetRef))...)
...
func crsEnvVars(cra *customresourcesselfdapternetv1.CustomResourcesSelfadapter, scaleTargetRef string) []corev1.EnvVar {
	envVars := []corev1.EnvVar{
		{
			Name:  "scaleTargetRef",
	...
	envVars = append(envVars, createEnvVarsFromConfig(cra.Spec.Config)...)
	return envVars
}

func createEnvVarsFromConfig(configs []customresourcesselfdapternetv1.ResourcesSelfadapterConfig) []corev1.EnvVar {
	envVars := []corev1.EnvVar{}
...
func (r *CustomResourcesSelfadapterReconciler) SetupWithManager(mgr ctrl.Manager) error {
	return ctrl.NewControllerManagedBy(mgr).
		For(&customresourcesselfdapternetv1.CustomResourcesSelfadapter{}).
		Named("customresourcesselfadapter").
