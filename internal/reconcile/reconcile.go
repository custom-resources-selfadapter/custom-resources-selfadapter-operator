...
import (
	...
	customresourcesselfadapterv1 "github.com/custom-resources-selfadapter/custom-resoursces-selfadapter-operator/api/v1"
...
func (k *KubernetesResourceReconciler) Reconcile(
	...
	instance *customresourcesselfadapterv1.CustomResourcesSelfadapter,
	obj metav1.Object,
	...
	// Set ScriptScaler instance as the owner and controller
	err := controllerutil.SetControllerReference(instance, obj, k.Scheme)
	if err != nil {
	...
	if !skaOwner {
		reqLogger.Info("SKA not set as owner, updating owner reference", "Kind", kind, "Namespace", obj.GetNamespace(), "Name", obj.GetName())
...
func (k *KubernetesResourceReconciler) PodCleanup(reqLogger logr.Logger, instance *customresourcesselfadapterv1.CustomResourcesSelfadapter) error {
	pods := &corev1.PodList{}
	err := k.Client.List(context.Background(), pods,
		client.MatchingLabels{controller.OwnedByLabel: instance.Name},
...
func (k *KubernetesResourceReconciler) deleteOrphan(reqLogger logr.Logger, pod corev1.Pod) error {
	reqLogger.Info("Found orphaned Pod (owned by SKA but not currently defined), deleting", "Kind", pod.GetObjectKind().GroupVersionKind(), "Namespace", pod.GetNamespace(), "Name", pod.GetName())
